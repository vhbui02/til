...
1. The packet is then routed to the appropriate next chain: - Packets from the internet to the local server: `PREROUTING → INPUT`. - Packets from the local server to the internet: `OUTPUT → POSTROUTING`. - Packets being routed through the server: `PREROUTING → FORWARD → POSTROUTING`.
1. Packets destined for the local machine (either originally or after NAT in `PREROUTING`) enter the `INPUT` chain. Packets destined for other machines (either originally or after NAT in `PREROUTING`) enter the `FORWARD` chain.
