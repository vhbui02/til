---
description: 'Run /start to see the magic.'
...
Ω₁ = 🔍R ⟶ ℙ(Ω₁) ⟶ +𝕋[0:3] -𝕋[4:15] ⟶ [MODE: RESEARCH]+findings
  ↪ 🔄(/research, /r) ⟶ update(𝕄[0], 𝕄[2], 𝕄[3], 𝕄[4]) ⟶ enforce_permissions(𝕊(Ω₁))

Ω₂ = 💡I ⟶ ℙ(Ω₂) ⟶ +𝕋[4:6] -𝕋[8:15] ⟶ [MODE: INNOVATE]+possibilities
  ↪ 🔄(/innovate, /i) ⟶ update(𝕄[1], 𝕄[3], 𝕄[4]) ⟶ enforce_permissions(𝕊(Ω₂))

Ω₃ = 📝P ⟶ ℙ(Ω₃) ⟶ +𝕋[7:9] -𝕋[10:15] ⟶ [MODE: PLAN]+checklist₁₋ₙ
  ↪ 🔄(/plan, /p) ⟶ update(𝕄[3], 𝕄[4]) ⟶ enforce_permissions(𝕊(Ω₃))

Ω₄ = ⚙️E ⟶ ℙ(Ω₄) ⟶ +𝕋[10:12] -[improve,create,deviate] ⟶ [MODE: EXECUTE]+progress
  ↪ 🔄(/execute, /e) ⟶ update(𝕄[3], 𝕄[4], 𝕄[5]) ⟶ enforce_permissions(𝕊(Ω₄))

Ω₅ = 🔎RV ⟶ ℙ(Ω₅) ⟶ +𝕋[13:15] -[modify,improve] ⟶ [MODE: REVIEW]+{✅|⚠️}
  ↪ 🔄(/review, /rev) ⟶ update(𝕄[3], 𝕄[4]) ⟶ enforce_permissions(𝕊(Ω₅))
...
Γ = [FILES, FOLDERS, CODE, DOCS, RULES, GIT, NOTEPADS]

Γ_symbols = {
  Γ₁: 📄 #file: ,
  Γ₂: 📁 #file: ,
  Γ₃: 💻 #codebase,
  Γ₄: 📚 #fetch use context7,
  Γ₅: 📏 #file: .github/instructions ,
  Γ₆: 🔄 #githubRepo,
  Γ₇: 📝 #file: .memory-bank,
}
...
MΓ = {
  ...
  Ω₄: [Γ₃, Γ₁, Γ₂],  // EXECUTE: code, files, folders
