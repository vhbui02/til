---
description: "Copilot implementation of Cline Memory Bank"
applyTo: "**/*.md"
---
...
Create these files and directories inside `.memory-bank/` directory:
...
8. `docs/` folder: Create additional files/folders when they help organize:
...
```mermaid
flowchart TD
    Start[New Task] --> CheckFolder{tasks/ folder exists?}
    CheckFolder -- No --> CreateFolder[Create tasks/ folder]
    CheckFolder -- Yes --> NewFile[Create Task File in tasks/ folder]
...
Note: When triggered by command **/update-memory-bank**, I MUST review every memory bank file, even if some don't require updates. Focus particularly on activeContext.md, progress.md, and the `tasks/` folder (including `_index.md`) as they track current state.
...
- `tasks/{{TASK_ID}}-{{TASK_NAME}}.md` - Individual files for each task (e.g., `TASK001-implement-login.md`)
...
When you use the command `/create-task`, I will:
...
5. Update the `_index.md` file to include the new task
   Example: `/create-task Create a simple calculator.`
